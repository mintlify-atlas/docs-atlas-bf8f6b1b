...
Queries are read-only server functions that run inside a `QueryCtx`. The context provides `ctx.db` (a `DatabaseReader`) for accessing documents and `ctx.auth` for the caller's identity. Queries cannot write to the database — any attempt to call a write method inside a query is a TypeScript error. Permission rules are enforced automatically: documents that the caller is not allowed to read are silently excluded from results.
