...
`createWorker` accepts a `BaseflareManifest` and returns the `fetch` handler object that Cloudflare expects as the Worker's default export. The CLI generates the Worker entry point automatically on deploy, but the pattern looks like this:
