...
- **`ctx.scheduler`** _(planned — Phase 7b)_ — schedule deferred work via `ctx.scheduler.runAfter()` and `ctx.scheduler.runAt()`.
- **`ctx.storage`** _(planned — Phase 7a)_ — access file storage via `ctx.storage.store()`, `ctx.storage.getUrl()`, and related methods.
